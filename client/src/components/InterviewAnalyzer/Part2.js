...
import { getUser, stopRecordingFunction, setRandomTest } from '../../actions/index';
...
import '../InterviewSimulator/testscreen.css';
...
function pixelate(sample_size){
  ...
  for (var y= 0; y < canvas.height; y+= sample_size) {  
    for (var x = 0; x < canvas.width; x+= sample_size) {     
     ...
     context.fillStyle = rgba(red, blue, green, 0.9);
...
class Part2 extends React.Component {
    constructor(props) {
        ...
        props.setRandomTest(val);
        const data2 = testarray[val];
        this.state = {
            redirect: false,
            questions: data2,
            optionValue: '',
  ...
  startTime() {
    ...
    render () {
        ...
        if(this.state.redirectToSuccess) {
            return (
                <Redirect to = {`/interviewanalyzer/part3?id=${ this.state.testId }`} />
            );
        }
            ...
            <Grid container style={{textAlign:"center"}}>
                    ...
                    <Grid item xs={12} style={{textAlign:"right"}}>
                        <div style={{display:"flex",justifyContent:"flex-end"}}>
                            <div className="timing">
                                <Typography style={{fontSize: "20px"}}>{this.state.time < 180 ? this.secondsToTime(this.state.time) : this.state.time -180 + "s"}</Typography>
                            </div>
                        </div>
                        {this.state.time < 180 ? <Typography style={{marginRight:"50px"}}>Answering Time</Typography> : <Typography style={{marginRight:"50px"}}>Preparation Time</Typography>  }
                        {this.state.time === 0 ? this.nextQues() : null  }
                    </Grid>
                </Grid>
                <Grid item xs={12} style={{marginBottom:"10px"}}>
                    <Typography style={{marginLeft:"40px",marginBottom:"-30px", textAlign:"left"}} variant="h4">Part 2</Typography>
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems:'flex-end', padding: '0px' }}>
                ...
                <Grid item xs={12} style={{margin:"40px 0"}}>
                    <div style={{textAlign:"center"}} >
                        ...
                        <Typography>
                           1. {this.state.questions.hint_1}
                        </Typography>
                        <Typography>
                           2. {this.state.questions.hint_2}
                        </Typography>
                        <Typography>
                           3. {this.state.questions.hint_3}
                        </Typography>
                        <Typography>
                            {this.state.questions.statement}
                        </Typography>
                        <audio
                            id="audioElement"
                            style={{ display: 'none' }}
                            autoplay="true"
                            controls
                            src={this.state.questions.audio}>
                        </audio>
