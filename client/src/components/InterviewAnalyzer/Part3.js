...
class Part3 extends React.Component {
                ...
                .then(res => {
                    if (res.status === 200) {
                    	axios.post("/api/completeTest", {_id:this.props.userData._id,testname:this.state.testName})
                    		.then(res => {
                    			if(res.status === 200) {
                                    var base_url = `https://innovative-labs.s3.ap-south-1.amazonaws.com/video/${this.props.userData.name}/${this.state.testname}/Test${this.props.userData.module - this.props.userData.test + 1}/Part`;
                                    var data = {
                                        0: base_url + "1.mp4",
                                        1: base_url + "2.mp4",
                                        2: base_url + "3.mp4"
                                    };
                                    axios.post("https://video-analysis-api.herokuapp.com", JSON.stringify(data))
                                        .then(result => {
    ...
    render () {
        ...
        if(this.state.redirectToSuccess) {
            return (
                <Redirect to = '/interviewanalyzer/test/complete' />
            );
        }
        ...
        <MuiThemeProvider theme={theme}>
            <Grid container style={{textAlign:"center"}}>
                ...
                <Grid item xs={12} style={{marginBottom:"10px"}}>
                    ...
                    <div style={{ display: 'flex', alignItems:'flex-end',justifyContent: 'center', padding: '0px' }}>
