...
import { getUser, brainLeftTestResult } from '../../actions/index';
...
class LeftBrainTest extends Component {
	constructor(props) {
		super(props);
		this.state = {
			questions: [],
		...
		if(this.props.userData) {
			if(this.props.userData.isVerified) {
				...
				axios.get("/api/test/" + testName).then(res => {
					var index = this.props.mode === "English" ? 0 : 2;
	...
	handleClick = () => {
		...
		if(hasAnswered) {
			...
			if(nextQues < this.state.questions.length) {
				...
				this.props.brainLeftTestResult(this.state.result);
	...
	render() {
		...
		if(this.state.redirect) {
			return <Redirect to="/brainmirror/test/rightbrain/start" />
		}
				...
				<Grid container>
					...
					<Grid style={{height:"75vh"}} item xs={3}>
						<div className="leftbrain-animation-container">
							...
							<BrainAnimation />
							<Typography variant="h5" className="brainmirror-heading">{this.props.mode === "English" ? "Analysing left side of the brain" : "मस्तिष्क के बाईं ओर का विश्लेषण"}</Typography>
						</div>
					</Grid>
				</Grid>
			</div>
...
function mapDispatchToProps (dispatch) {
    return bindActionCreators({
        getUser: getUser,
        brainLeftTestResult: brainLeftTestResult
    }, dispatch)
};

export default connect(mapStateToProps, mapDispatchToProps)(LeftBrainTest);